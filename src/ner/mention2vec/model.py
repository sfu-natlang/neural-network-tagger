...
class NERModel(object):
    ...
    def add_logits_op(self):
        ...
        with tf.variable_scope("proj"):
            ...
            self.boundry_logits = tf.reshape(pred, [-1, ntime_steps, self.niob])

        with tf.variable_scope("type"):
            to_concat = []
            batch_size = tf.shape(self.mention)[0]
            ...
            cell_fw1 = tf.contrib.rnn.LSTMCell(2*self.config.hidden_size)
            cell_bw1 = tf.contrib.rnn.LSTMCell(2*self.config.hidden_size)
            ...
            W1 = tf.get_variable("W1", shape=[4*self.config.hidden_size, self.ntype], dtype=tf.float32)
    ...
    def predict_iob_batch(self, sess, words):
        """
        Args:
    ...
    def predict_type_batch(self, sess, words, mentions, mention_size):
        fd, _ = self.get_feed_dict(words, mentions=mentions, mention_size=mention_size, dropout=1.0)
