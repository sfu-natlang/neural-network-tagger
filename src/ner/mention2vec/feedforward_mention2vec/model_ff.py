...
from utils import minibatches, pad_sequences, get_chunks, find_mentions, find_labels, load_vocab
import time
import codecs


class NERModel(object):
  def __init__(self, config, embeddings, ntags, nchars=None, niob=None, ntype=None, id2type=None):
    """
    Args:
  ...
  def get_feed_dict(self, words, word_features, char_ids=None, lr=None, dropout=None, iob=None, mention_type=None, mentions=None):
    """
  ...
  def add_word_embeddings_op(self):
    ...
    with tf.variable_scope("chars"):
      if self.config.chars:
        ...
        output = tf.reshape(output, shape=[-1, 8, 2*self.config.char_hidden_size])

    if self.config.chars:
      word_embeddings = tf.reshape(word_embeddings, shape=[-1, 8, self.config.dim])
      self.word_embeddings = tf.concat([word_embeddings, output], axis=-1)
    else:
  ...
  def add_logits_op(self):
    ...
    with tf.variable_scope("ff"):
      ...
      self.word_embeddings = tf.reshape(self.word_embeddings, shape=[-1, input_size])
  ...
  def add_train_op(self):
    ...
    with tf.variable_scope("train_step"):
      ...
      if self.config.clip > 0:
        ...
        self.train_op_boundry = optimizer.apply_gradients(zip(gradients, variables))
      else:
        self.train_op = optimizer.minimize(self.loss)
        self.train_op_boundry = optimizer.minimize(self.loss_a)                
  ...
  def predict_iob_batch(self, sess, words, word_features, char_features):
    """
    Args:
    ...
    fd, sequence_lengths, _ = self.get_feed_dict(words, word_features, char_features, dropout=1.0)
  ...
  def predict_type_batch(self, sess, words, word_features, char_features, mentions):
    fd, _, mention_sizes = self.get_feed_dict(words, word_features, char_features, mentions=mentions, dropout=1.0)
