...
class EntityLSTM(object):
    ...
    def __init__(self, dataset, parameters):
        ...
        self.input_label_indices = tf.placeholder(tf.int32, [None], name="input_ner_indices")
        self.input_pos_indices = tf.placeholder(tf.int32, [None], name="input_pos_indices")
        self.input_token_character_indices = tf.placeholder(tf.int32, [None, None], name="input_token_char_indices")
        ...
        if parameters['use_tag_embedding']:
        # tag embedding layer
            with tf.variable_scope("tag_embedding"):
                self.tag_embedding_weights = tf.get_variable(
                    "tag_embedding_weights",
                    shape=[dataset.pos_classes, parameters['pos_embedding_dimension']],
                    initializer=initializer)
                embedded_pos = tf.nn.embedding_lookup(self.tag_embedding_weights, self.input_pos_indices)
        ...
        if parameters['use_tag_embedding']:
            token_lstm_input = tf.concat([token_lstm_input, embedded_pos], axis=1, name='token_pos_lstm_input')
        ...
        with tf.variable_scope('token_lstm') as vs:
            ...
            #cell_forward = tf.contrib.rnn.MultiRNNCell([cell_forward] * config.num_layers, state_is_tuple=True)
            #cell_backward = tf.contrib.rnn.MultiRNNCell([cell_backward] * config.num_layers, state_is_tuple=True)
            #outputs, _ = tf.nn.bidirectional_dynamic_rnn(cell_forward, cell_backward, token_lstm_input_drop_expanded, sequence_length=self.sentence_length, dtype=tf.float32)
            #outputs_forward, outputs_backward = outputs
            #token_lstm_output = tf.concat([outputs_forward, outputs_backward], axis=2, name='output_sequence')
