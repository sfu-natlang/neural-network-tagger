...
class GreedyTagger(object):
  ...
  def _BuildNetwork(self, feature_endpoints, return_average=False):
    ...
    for i in range(self._feature_size):
      embeddings.append(self._AddEmbedding(feature_endpoints[i],
                                           ...
                                           i,
                                           return_average=return_average))
    logging.info(tf.shape(embeddings[0]))
    last_layer = tf.concat(values=embeddings, concat_dim=1)
  ...
  def AddTraining(self,
    ...
    with tf.name_scope('training'):
      nodes = self.training
      nodes.update(self._BuildNetwork(self.input,
                                      return_average=False))
      nodes.update(self._AddCostFunction(batch_size, self.labels,
                                         nodes['logits']))
      # Add the optimizer
      if self._only_train:
        trainable_params = [v
