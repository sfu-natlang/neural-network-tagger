...
class Sentence():
  ...
  def __init__(self, column_list={}, field_name_list=[]):
    ...
    self.output_tags = []
    self.epoch = -1
    self.feat_gen = Feat_Gen(self.get_word_list(), self.get_pos_list())
  ...
  def gen_sent_features(self, word_map, pos_map, prefix_map=None, suffix_map=None):
    self.word_features, self.cap_features, self.prefix_features, self.suffix_features, self.other_features = self.feat_gen.gen_word_features(word_map, pos_map, prefix_map, suffix_map)

  def get_history_features(self):
    return self.feat_gen.get_history_features(self.cur_tags)

  def gen_id_list(self, word_map, pos_map, char_map=None):
